...
class LEDMatrixController:
    ...
    def __init__(self, port: str, baud_rate: int = 1000000, matrix_size: tuple = (40, 40)):
        """
        ...
        Args:
            ...
            matrix_size: Dimensions of the LED matrix (width, height)
        """
        self.port = port
        self.baud_rate = baud_rate
        self.matrix_size = matrix_size
        self.num_pixels = matrix_size[0] * matrix_size[1]
        self.ser = None
    ...
    def send_frame(self, frame: np.ndarray, wait_for_ack: bool = True) -> bool:
        ...
        if frame.shape != (self.matrix_size[1], self.matrix_size[0], 3):
            print(f"Frame has wrong dimensions: {frame.shape}, expected {(self.matrix_size[1], self.matrix_size[0], 3)}")
            return False
...
if __name__ == "__main__":
    # Example of creating test patterns
    def create_test_pattern(pattern_type="rainbow"):
        matrix = np.zeros((40, 40, 3), dtype=np.uint8)
        ...
        if pattern_type == "rainbow":
            ...
            for y in range(40):
                for x in range(40):
                    ...
                    if hue < 85:
                        matrix[y, x] = [255 - hue * 3, hue * 3, 0]
                    elif hue < 170:
                        hue -= 85
                        matrix[y, x] = [0, 255 - hue * 3, hue * 3]
                    else:
                        hue -= 170
                        matrix[y, x] = [hue * 3, 0, 255 - hue * 3]
            ...
            for y in range(40):
                for x in range(40):
                    if (x // 5 + y // 5) % 2 == 0:
                        matrix[y, x] = [255, 0, 0]  # Red
                    else:
                        matrix[y, x] = [0, 0, 255]  # Blue
        
        return matrix
    
    # Create an animated test video (10 frames of shifting rainbow)
    def create_test_video(frames=10):
        video = np.zeros((frames, 40, 40, 3), dtype=np.uint8)
        for i in range(frames):
            pattern = create_test_pattern("rainbow")
            # Shift the pattern for each frame
            video[i] = np.roll(pattern, i * 3, axis=0)
        return video
    
    # Connect and send test pattern
    try:
        # Replace 'COM3' with your Arduino's serial port
        controller = LEDMatrixController(port='/dev/ttyACM0')  # Use 'COM3' on Windows
        
        if controller.connect():
            # Create and send a test video
            test_video = create_test_video(frames=24)  # 1 second of animation at 24fps
